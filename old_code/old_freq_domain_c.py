...
def convert_old_syntax(p_prec):
    try:
        p_prec['wmin'] = p_prec['fmin']
        p_prec['wmax'] = p_prec['fmax']

        del p_prec['fmin']
        del p_prec['fmax']

        message = "\n\n%s(DEPRECATED)%s\n"\
                  "The use of the keys 'fmin' and 'fmax' in Fw.p_prec is deprecated and support "\
                  "will be removed in a future version. "\
                  "Use 'wmin' and 'wmax' instead.\n" % (bcolors.WARNING, bcolors.ENDC)
...
class FwGeneral_C():
    ...
    def __init__(self, It, p_prec={}):
        ...
        p_prec = convert_old_syntax(p_prec)
...
class Fw_FFT(FwGeneral_C):
    """(in preparation)"""
    def __init__(self, It, p_prec={}):
        super().__init__(It, p_prec)

        self.name = 'FFT (new regularization)'
        self.windows_dic = {}

        # split It into regular/singular contributions and interpolate
        # the regular part
        self.It_reg_grid = self._update_RegScheme(self.p_prec['reg_stage'], It.t_grid, It.It_grid)
    ...
    def tau_range(self, fmin, fmax):
        """
        Compute the array of taus to evaluate the function
        """
    ...
    def compute_standard(self, wmin, wmax, eval_It):
        """
        Standard FFT between a maximum and minimum frequency
        """
    ...
    def compute_multigrid(self, wmin, wmax, eval_It):
        """
        Divide the frequency range into bits of (logarithmic) size N_keep.
        For each of them, call 'compute_standard' adding a number of decades
        in frequency above and below. These are only used to increase the
        precision in the range N_keep and are later discarded.
        """
...
## ********************************************************************
## keep support for old syntax

class Fw_SL(Fw_FFT):
    def __init__(self, It, p_prec={}):
        message = "the syntax 'Fw_SL' is deprecated. Use 'Fw_FFT' instead."
        warnings.warn(message, FreqDomainWarning)
        super().__init__(It, p_prec)

class Fw_SL_C(Fw_FFT_C):
    def __init__(self, It, p_prec={}):
        message = "the syntax 'Fw_SL_C' is deprecated. Use 'Fw_FFT_C' instead."
        warnings.warn(message, FreqDomainWarning)
        super().__init__(It, p_prec)
## ********************************************************************
