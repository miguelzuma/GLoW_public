...
class ItGeneral_C():
    ...
    def __init__(self, Lens, y, p_prec={}):
        ...
        self.lens_to_c = self.check_lens(self.lens)
    ...
    def check_lens(self, lens):
        ...
        -------
        lens : PsiGeneral subclass
            Lens object that will be send to the C code.
        """
        if wrapper.check_implemented_lens(lens) is False:
            message = "lens '%s' not implemented in C, proceeding numerically" % lens.p_phys['name']
            warnings.warn(message, TimeDomainWarning)

            if lens.isAxisym is False:
        ...
        elif lens.p_phys['name'] == 'combined lens':
            # If not all the lenses are implemented, we create a new object.
            # Not great, but I wanted to avoid:
            #  1) overwriting the original Python class provided by the user
            #  2) replicating a (maybe long) list of sublenses if not needed

            id_list_new = []
            lens_list_new = []
            all_implemented = True
            for i, l in enumerate(lens.p_phys['lenses']):
                if wrapper.check_implemented_lens(l) is False:
                    all_implemented = False
                    id_list_new.append(i)
                    lens_list_new.append(self.check_lens(l))

            if all_implemented is False:
                lens_list = lens.p_phys['lenses'][:]
                for i, l in zip(id_list_new, lens_list_new):
                    lens_list[i] = l

                lens = lenses.CombinedLens({'lenses':lens_list})
